function [phi,aux_seg,c]= lsm_classical_3D(Ib,init_mask,max_its,min_its,p,subj,dx,c_min,mask_tot,pos)
...
if pos==1
    nome_video=sprintf(['Video_sol/Video_fdm_3D_subj_',subj,'_p',num2str(p),'_s.avi']);
elseif pos==2
    nome_video=sprintf(['Video_sol/Video_fdm_3D_subj_',subj,'_p',num2str(p),'_d.avi']);
end
...
while its<min_its || (its <=max_its)
    %--
    %-- evolve the curve
    phi(ind_phi) = S_m(phi,ind_phi,c,dt,dx);
...
function v = H(c,p,q,r)
v=c.*sqrt(p.*p+q.*q+r.*r);
end

function v = Hp(c,p,q,r)
v=c.*p./sqrt(p.*p+q.*q+r.*r+eps);
end

function v = Hq(c,p,q,r)
v=c.*q./sqrt(p.*p+q.*q+r.*r+eps);
end

function v = Hr(c,p,q,r)
v=c.*r./sqrt(p.*p+q.*q+r.*r+eps);
end

%Local Lax-Friedrichs scheme
function v = G_lf(c,u_m,u_p,v_m,v_p,w_m,w_p)
H_p=abs(Hp(c,u_m,0,0));
ind=abs(Hp(c,u_p,0,0))>H_p;
H_p(ind)=abs(Hp(c(ind),u_p(ind),0,0));
H_q=abs(Hq(c,0,v_m,0)); 
ind=abs(Hq(c,0,v_p,0))>H_q;
H_q(ind)=abs(Hq(c(ind),0,v_p(ind),0));
H_r=abs(Hr(c,0,0,w_m));
ind=abs(Hr(c,0,0,w_p))>H_r;
H_r(ind)=abs(Hr(c(ind),0,0,w_p(ind)));
v=H(c,0.5*(u_m+u_p),0.5*(v_m+v_p),0.5*(w_m+w_p))-0.5*(H_p.*(u_p-u_m)+H_q.*(v_p-v_m)+H_r.*(w_p-w_m));
...
%  v=H(1,0.5*(u_m+u_p),0.5*(v_m+v_p),0.5*(w_m+w_p))-0.5*(u_p-u_m)-0.5*(v_p-v_m)-0.5*(w_p-w_m);
  ...
  function v = S_m(phi,ind_phi,c,dt,dx)
...
%   %-- compute Hamiltonian    
    ...
    v=phi(ind_phi) - dt.*G_lf(c(ind_phi),phi_xm,phi_xp,phi_ym,phi_yp,phi_zm,phi_zp);  
